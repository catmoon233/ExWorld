...
public final class RegionPersistenceTestHarness {
    public static void main(String[] args) {
        ...
        require(loaded.equals(source), "group name, icon, site, resources or visibility did not survive persistence");
        Region legacy = WorldNbtCodec.loadRegion(WorldNbtCodec.saveRegion(new Region("old", List.of("tile_p0_p0"), "旧区", 1L)));
        require(!legacy.configured() && legacy.site().isEmpty() && legacy.resources().isEmpty(),
                "unset generated groups must remain hidden after persistence");
