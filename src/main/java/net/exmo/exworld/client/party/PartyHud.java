...
public final class PartyHud {
    ...
    private static void render(GuiGraphics g){Minecraft mc=Minecraft.getInstance();if(net.exmo.exworld.Config.decryptionMode)return;var snapshot=PartyClient.snapshot();if(BattleClient.active()||mc.options.hideGui||mc.player==null||snapshot.members().size()<=1)return;
