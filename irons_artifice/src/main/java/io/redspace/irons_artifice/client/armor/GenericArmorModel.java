...
public class GenericArmorModel<T extends Item & GeoItem> extends DefaultedItemGeoModel<T> {
    ...
    public GenericArmorModel(String modid, String name) {
        this(
                ResourceLocation.fromNamespaceAndPath(modid, "geo/armor/" + name + ".geo.json"),
                ResourceLocation.fromNamespaceAndPath(modid, "textures/models/armor/" + name + ".png")
        );
    }

    public GenericArmorModel(ResourceLocation model, ResourceLocation texture) {
        super(ResourceLocation.fromNamespaceAndPath(model.getNamespace(), "empty"));
        this.model = model;
