...
public final class CrosshairRenderer {
    ...
    public static boolean renderGunCrosshair(GuiGraphics graphics, DeltaTracker deltaTracker) {
        ...
        float fov = minecraft.options.fov().get() * (GunItem.isScoping(player) ? GunScopeOverlay.FOV_MODIFIER : player.getFieldOfViewModifier());
        float gap = Math.max(GAP_BASE + degreesToGuiPixels(degreesSpread, graphics.guiHeight(), fov), 0);
        ...
        if (reloadAnimationTick > 0) {
            ...
            poseStack.mulPose(Axis.ZP.rotationDegrees(f * 180));
            poseStack.translate(-0.5f, -0.5f, 0);
        }
