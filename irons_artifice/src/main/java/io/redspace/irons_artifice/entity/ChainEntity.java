...
public class ChainEntity extends Entity {
    ...
    @Nullable
    private java.util.UUID firstUuid;
    @Nullable
    private java.util.UUID secondUuid;
    ...
    @Nullable
    private LivingEntity resolveBound(int entityId, @Nullable java.util.UUID uuid) {
        if (entityId != 0) {
    ...
    protected void readAdditionalSaveData(CompoundTag input) {
        if (input.hasUUID("First")) {
            this.firstUuid = input.getUUID("First");
        }
        if (input.hasUUID("Second")) {
            this.secondUuid = input.getUUID("Second");
        }
        this.tickCount = input.getInt("Age");
        if (level() instanceof ServerLevel serverLevel) {
            if (firstUuid != null && serverLevel.getEntity(firstUuid) instanceof LivingEntity first) {
                entityData.set(DATA_FIRST_ID, first.getId());
            }
            if (secondUuid != null && serverLevel.getEntity(secondUuid) instanceof LivingEntity second) {
